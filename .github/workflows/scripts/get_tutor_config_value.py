"""Extract a specific variable value from the config.yml file in the current directory and print it to the console so it can be
used by GitHub Actions steps by using the following syntax:
...
def get_config_value(variable_name, config_file_path):
    ...
    Args:
        variable_name (str): The name of the variable to get from the config.yml file.
    ...
    if not value:
        print(f"ERROR: {variable_name} not found in the given config.yml", file=sys.stderr)
        sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("ERROR: Please provide the variable name to extract from config.yml", file=sys.stderr)
        sys.exit(1)
